...
def reverse_next_questions():
    ...
    try:
        resp = client.messages.create(
            model=MODEL, max_tokens=300,
            messages=[{"role": "user", "content":
                f"あなたは「なんでもあてっこゲーム」でお題を当てようとしているプレイヤーのアシスタントです。\n"
                f"お題が何かはわかりません。これまでの質問と回答から判断して、次に効果的な質問候補を6つ提案してください。\n\n"
                f"これまでの質問数：{question_count}問\n"
                f"これまでの質問と回答:\n{history_text}\n\n"
                f"【質問生成のルール】\n"
                f"- 質問は段階的に絞り込む\n"
                f"- 仮説が先走った質問（まだ根拠がないのに「人間ですか？」など）はしない\n"
                f"- 各質問はすでに聞いた視点と重複しない独立した観点から聞く\n"
                f"- {question_count}問以下の段階では：大カテゴリ（生き物か？物か？場所か？人か？食べ物か？現象か？）を絞る質問を優先する\n"
                f"- 4〜6問の段階では：中カテゴリ（日本のもの？大きい？有名？）を絞る質問\n"
                f"- 7問以降は：具体的な特徴を聞く質問\n\n"
                f"小学生向けのやさしい言葉で、はい/いいえで答えられる質問を6つ生成してください。JSONで返してください。\n"
